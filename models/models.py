...
class MySmote(object):
    ...
    def fit_resample(self, X, y=None, n_final_sample=None):
        ...
        #le premier élément sera tjrs le point en lui-même vu qu'on a fit sur X aussi.
        ...
        for i in range(n_synthetic_sample):
            ...
            k1 = np.random.randint(1,self.K+1)# Sélection voisin parmi les K. On exclu 0 car c indice lui-même
            indice_neighbor = neighbor_by_index[indice][k1]
            w = self.w_simulation(**self.w_simulation_params) #facteur alpha de la première difference
...
class CVSmoteModel(object):
    ...
    def __init__(self, splitter, model, list_k_neighbors=None, arange_max=100,arange_step=10):
        self.splitter = splitter
        self.arange_max = arange_max
        self.arange_step = arange_step
        self.model = model
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        list_k_neighbors.extend(list(
            np.arange(1,self.arange_max,self.arange_step,dtype=int)))
...
class CVSGModel(CVSmoteModel):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for k in list_k_neighbors:
            scores = []
            for train, test in self.splitter.split(X, y):
                new_X, new_y = smote_n_points_gauss_classif(X=X[train],y=y[train],n_points=k,K=k,llambda=1)
                self.model.fit(new_X, new_y, sample_weight)
                scores.append(roc_auc_score(y[test], self.model.predict_proba(X[test])[:,1]))
            if sum(scores) > best_score:
                best_k = k

        new_X, new_y = smote_n_points_gauss_classif(X=X,y=y,n_points=best_k,K=best_k,llambda=1)
        self.model.fit(new_X, new_y, sample_weight)
